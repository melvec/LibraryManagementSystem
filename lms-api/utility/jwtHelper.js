...
export const generateAccessJWT = (email) => {
  try {
    const accessToken = jwt.sign({ email }, process.env.ACCESS_JWT_SECRET_KEY, { expiresIn: "15m"} )
...
const generateRefreshJWT = (email) => {
  try {
    const refreshToken = jwt.sign({ email }, process.env.REFRESH_JWT_SECRET_KEY, { expiresIn: "30d"} )
